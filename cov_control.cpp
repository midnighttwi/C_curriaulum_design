...
void Cov_control::loadfile(const QString filename,vector<Coverage>& cov)//将文件中的数据存入图元类
{
    QFile file(filename);
...
void Cov_control::savefile(const QString filename,vector<Coverage>& cov)//将图层中的数据保存到文件中
{
        QFile file(filename);
...
void Cov_control::loadDatabase()//将数据库中的数据实例化为类
{

}
void Cov_control::saveDatabase()//将数据保存到数据库中
{
